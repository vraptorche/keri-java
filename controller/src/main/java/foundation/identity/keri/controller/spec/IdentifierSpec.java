...
public class IdentifierSpec {
  ...
  public static class Builder {
    ...
    public Builder keys(List<PublicKey> keys) {
      ...
      if (keys.isEmpty()) {
        throw new IllegalArgumentException("Public keys must be provided.");
      }
    ...
    public IdentifierSpec build() {
      ...
      if (this.signingThreshold instanceof SigningThreshold.Unweighted unw) {
        if (unw.threshold() > this.keys.size()) {
          throw new IllegalArgumentException(
              "Invalid unweighted signing threshold: keys: %s  threshold: %s".formatted(this.keys.size(), unw.threshold()));
        }
        ...
        if (countOfWeights != this.keys.size()) {
          throw new IllegalArgumentException(
              "Count of weights and count of keys are not equal: keys: %s weights: %s".formatted(this.keys.size(), countOfWeights));
        }
      } else {
      ...
      if (this.witnesses.size() != Set.copyOf(this.witnesses).size()) {
        throw new IllegalStateException("List of witnesses has duplicates");
      }
